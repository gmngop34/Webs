...
function filterByCategory(categoryName) {
    ...
    // If we are on index.html, we can filter locally without redirecting 
    // to "All Movies" every time, unless you prefer it that way.
    const onHome = window.location.pathname.includes('index.html') || window.location.pathname === "/";
    
    if (!onHome && !window.location.pathname.includes('movies.html')) {
    ...
    setTimeout(() => {
        ...
        const targetGrid = document.getElementById('featuredGrid') || document.getElementById('allMoviesGrid');
        
        if (targetGrid) {
            displayMovies(filtered, targetGrid.id);
            const titleElement = document.querySelector('.section-title');
            if(titleElement) titleElement.innerText = categoryName.toUpperCase() + " MOVIES";
        }
        
        toggleLoader(false); // Hide spinner after logic finishes
    }, 300);
}
